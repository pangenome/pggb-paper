...
The second step in PGGB is then the construction of the pangenome graph in the variation model.
...
Therefore, while the large-scale structure of the graph is correct, the base-level alignments suffer from the erosion of the small matches in the alignments.
...
We enforce this prior intuition by applying SMOOTHXG (SUPP. METHODS FOR ITS EXPLANATION), a tool that normalize the
...
Futhermore, it can optionally generate a multiple sequence alignment in the traditional MAF format, and VCF files against any chosen set of reference sequences embedded in the pangenome graph.
