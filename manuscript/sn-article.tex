...
\abstract{
    Advances in genome assembly have greatly reduced the cost of obtaining Telomere-to-Telomere (T2T) genome assemblies, but their unbiased analysis remains difficult.
    Pangenome graphs provides an efficient model to represent and analyse multiple genomes.
    In response to the future abundance of high-quality T2T assemblies, we propose the PanGenome Graph Builder (PGGB), a reference-free pangenome graph construction pipeline.
    We apply our method to several species of the tree of life and show that we can analyse hundreds of whole genomes in an unbiased manner.
}
...
\keywords{pangenome graphs, pangenome building, pangenomics, comparative genomics}
...
The study of all these high-quality assemblies has therefore opened the door for a comprehensive understanding of the genetic variability of populations.
...
To properlyu understand genome variability and evolution, we need to obtain an unbiased image of the relationship between multiple sequences.
...
%This produces a simple DAG, with a linear reference and a hierarchy of structural variation build out of it.A guide tree has sense when considering distantly-related groups, but it breaks down within species or in the case of incomplete lineage sorting,
...
To overcome the limitations of current pangenome building approaches, we implemented the PanGenomeGraph Builder (PGGB) pipeline.
PGGB renders a collection of sequences into a variation graph~\cite{Garrison2018} able to represent any kind of relationships between the input sequences.
...
PGGB consists in three main steps, i.e. sequence alignment, graph construction, and graph normalization, but our approach is modular and flexible.
...
To avoid any kind of genome reference and order bias, the first step in PGGB is the all-to-all alignment of the input sequences.
...
This allows us to remove bias from pangenome analysis.
The all-to-all alignment of genomes embedds their pluralistic homologies and it implies a variation graph.
The second step in PGGB is the construction of the pangenome graph in the variation model.
We apply SEQWISH~\cite{Garrison2022}, a tool constructor that renders a variation graph from a set of input sequences and alignments between them.
Like the alignment method we apply, this construction method leads to an unbiased pangenome graph.
Pangenome graphs produced directly from pairwise alignments contain very complex local structures that result from ambiguous alignments between repetitive sequences.
...
In result, the graphs may also be locally underaligned.
...
We enforce this prior intuition by applying SMOOTHXG (FIXME SUPP. METHODS FOR ITS EXPLANATION), a tool that normalize the
graph in one or multiple steps by running a partial order alignment (POA)~\cite{Lee2002} over small pieces of the graph, letting larger SVs remain nonlinear.
PGGB concludes with tools that generate 1D and 2D visualizations of the graph and its embedded paths, using the graph genome toolkit ODGI~\cite{Guarracino2021}.
We can optionally generate a multiple sequence alignment in the traditional MAF format, and VCF files against any chosen set of reference sequences embedded in the graph.
...
%fixme graph statistics, graph compression, precision/recall/F1-score
