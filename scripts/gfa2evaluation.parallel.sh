...
PATH_VCF_PREPROCESS=/lizardfs/guarracino/git/pggb/scripts/vcf_preprocess.sh
PATH_NUCMER_2_VCF=/lizardfs/guarracino/git/pggb/scripts/nucmer2vcf.R
...
zgrep '#CHROM' "$PATH_VCF".gz -m 1 | cut -f 10- | tr '\t' '\n' | while read HAPLO; do
  ...
  bcftools view -a -s "$HAPLO" -Ou "$PATH_WAVED_FIXED_VCF".gz \
      | bcftools norm -f "$PATH_REF_FA" -c s -m - -Ou \
      | bcftools view -v snps,mnps  -e 'GT="ref" | GT~"\."' -f 'PASS,.' -Ou \
      | bcftools sort -m 64G -T bcftools-sort.XXXXXX -Ou \
      | bcftools norm --threads 48 -a -d exact -Oz -o "$PREFIX"."$PREFIX_REFERENCE".haplo.waved.fixed."$HAPLO".max1.vcf.gz
  bcftools index -t "$PREFIX"."$PREFIX_REFERENCE".haplo.waved.fixed."$HAPLO".max1.vcf.gz
done
...
cut -f 1 "$PATH_SEQUENCES_FA_GZ".fai | grep "^${PREFIX_REFERENCE}#" -v | while read CONTIG; do
  ...
  PREFIX2=nucmer/"$CONTIG"
  samtools faidx "$PATH_SEQUENCES_FA_GZ" "$CONTIG" > "$PREFIX2".fa
  echo "$PREFIX2" >> tmp
done
...
cut -f 1 "$PATH_SEQUENCES_FA_GZ".fai | grep "^${PREFIX_REFERENCE}#" -v | while read CONTIG; do
  ...
  PREFIX2=nucmer/"$CONTIG"
  show-snps -THC "$PREFIX2".delta > "$PREFIX2".var.txt
  #show-snps -TH "$PREFIX2".delta | cut -f 1-6,9-12 > "$PREFIX2".var.txt # For taking also variants from ambiguous alignments

  # Check if there are variants
  if [[ $(wc -l "$PREFIX2".var.txt | cut -f 1 -d\ ) != 0 ]]; then
    Rscript "$PATH_NUCMER_2_VCF" "$PREFIX2".var.txt "$CONTIG" "$PATH_REF_FA" "$NUCMER_VERSION" "$PREFIX2".vcf
    bgzip -@ "$THREADS" -l 9 "$PREFIX2".vcf
    tabix "$PREFIX2".vcf.gz
  fi
done
...
zgrep '#CHROM' "$PATH_VCF".gz -m 1 | cut -f 10- | tr '\t' '\n' | while read HAPLO; do
  ...
  grep "^$HAPLO" "$PATH_SEQUENCES_FA_GZ".fai | cut -f 1 | while read CONTIG; do
    ...
    PREFIX2=nucmer/"$CONTIG"
    PATH_VCF2=$PREFIX2.vcf.gz
    if [[ -f "$PATH_VCF2" ]]; then
      # Check if there are valid variants
      if [[ $(zgrep '^#' "$PATH_VCF2" -v | grep "^$REF_PREFIX" | wc -l) != 0 ]]; then
        echo "$PATH_VCF2" >> "$HAPLO".x.txt
      fi
    fi
  done
  ...
  cat "$HAPLO".x.txt | while read FILE; do
    rm "$FILE"
  done
...
zgrep '#CHROM' "$PATH_VCF".gz -m 1 | cut -f 10- | tr '\t' '\n' | while read HAPLO; do
  ...
  NUM_VARIANTS_NUCMER_TP=$(grep None vcfeval/haplo.waved/"$HAPLO"/summary.txt | sed 's,/summary.txt:,,' | tr -s ' ' | cut -f 3 -d ' ')
