...
sed '1d' $PATH_PANGENOMES_TSV | sort -k 4n | while read f p s n k G POA O REF; do
  seq 1 1 | while read c; do
    ...
    sbatch -c $THREADS -p 386mem -J $(basename "$f" .fa.gz) --wrap "hostname; cd /scratch; $PGGB -i $f -p $p -s $s -n $n -k $k -P $POA -O $O -G $G -t $THREADS -o $out -x auto; mv $out $DIR_GRAPHS/$out;"
  done
done
...
sed '1d' $PATH_PANGENOMES_TSV | grep 'ecoli500' | sort -k 4n | while read f p s n k G POA O REF; do
  seq 1 1 | while read c; do
    echo $f $p $s $n $k $G $POA $O $REF $c;
...
mkdir -p /lizardfs/guarracino/pggb-paper/assemblies/scerevisiae
mv /lizardfs/guarracino/pggb-paper/assemblies/scerevisiae142.fa.gz /lizardfs/guarracino/pggb-paper/assemblies/scerevisiae
cd $DIR_GRAPHS/
ls scerevisiae142_p9*/*log | while read f; do sed 's,/lizardfs/guarracino/pggb-paper/assemblies/scerevisiae142.fa.gz,/lizardfs/guarracino/pggb-paper/assemblies/scerevisiae/scerevisiae142.fa.gz,g' -i $f; done
# AND PREPARE FILES ALSO FOR SCEREVISIAE142

sed '1d' $PATH_PANGENOMES_TSV | grep athaliana7 -v | grep athaliana81 -v | grep scerevisiae142 -v | cut -f 1 -d ' ' | sort | uniq | while read PATH_FASTA; do
...
# Fix references
to do xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
