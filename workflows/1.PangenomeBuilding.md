...
PATH_FASTA=/lizardfs/guarracino/pggb-paper/assemblies/mmusculus/mmusculus36.chr19.fa.gz
GFA_GZ=mmusculus36.chr19.gfa.gz

PATH_FASTA=/lizardfs/guarracino/pggb-paper/assemblies/hsapiens/hsapiens90.chr6.masked.fa.gz
GFA_GZ=hsapiens90.chr6.masked.gfa.gz

NAME=$(basename $GFA_GZ .gfa.gz)

awk -v OFS='\t' '{print($1,"0",$2)}' $PATH_FASTA.fai > $NAME.all.bed
zgrep "^W" $GFA_GZ | cut -f 2,3,4,5,6 | awk -v OFS='\t' '{print($1"#"$2"#"$3,$4,$5)}' | sed 's/#0#/#1#' > $NAME.kept.bed

(echo "#sequence length.bp removed.bp removed.ratio" | tr ' ' '\t'; join \
  <(cut -f 1,3 $NAME.all.bed | sort ) \
  <(awk 'BEGIN{while(getline < "'$NAME.all.bed'") sum[$1]=0} {sum[$1] += $3 - $2} END{for(key in sum) print key, sum[key]}' <(bedtools subtract -a $NAME.all.bed -b $NAME.kept.bed) | sort) | awk -v OFS='\t' '
    {
        split($1, arr, "#");
        key = arr[1] "#" arr[2];
        length_sum[key] += $2;
...
## PGGB with/without smoothxg # quantify the runtime and memory too for odgi build/extract/sort????????????????????????????????????????????????
