...
sed '1d' $PATH_PANGENOMES_TSV | grep athaliana82 | sort -k 4n | while read f p s n k G POA O REF; do
...
ls $DIR_BASE/vcfs/hsapiens/*vcf.gz | while read VCF; do
  echo $VCF
  NAME=$(basename $VCF .vcf.gz | sed 's/_EBV//g')
  zcat $VCF | sed 's/chr/chm13#1#chr/g' | grep 'chrEBV' -v | bgzip -@ 48 -l 9 > $NAME.vcf.gz
  rm $VCF
  tabix $NAME.vcf.gz
done

cut -f 1 /lizardfs/guarracino/robertsonian_translocation/assemblies/chm13v2.0.fa -d ' ' | sed 's/>chr/>chm13#1#chr/g' > $DIR_BASE/vcfs/hsapiens/chm13v2.fa
rtg format -o $DIR_BASE/vcfs/hsapiens/chm13v2.fa.sdf $DIR_BASE/vcfs/hsapiens/chm13v2.fa
