...
sed '1d' $PATH_PANGENOMES_TSV | sort -k 4n | grep soy | grep chr14 | grep 10000 | head -n 1 | while read f p s n k G POA O REF; do
...
#for SPECIES in athaliana82 tomato23 soy37; do
  ...
  sed '1d' $PATH_PANGENOMES_TSV | sort -k 4n | grep $SPECIES | while read f p s n k G POA O REF; do
    ...
    echo $SUFFIX
    ls $DIR_GRAPHS/$SPECIES*$SUFFIX/*final.og | tr ' ' '\n' > $SUFFIX.tmp.txt
...
#reorder_seqfile "$DIR_BASE/assemblies/soy/soy37.seqfile" "ZH13" ###########################################################################
...
sed '1d' $PATH_PANGENOMES_TSV | grep 'athaliana82\|tomato23\|soy37' -v | grep 'mmusculus\|ecolo' | cut -f 1 -d ' ' | sort | uniq | while read PATH_FASTA; do
  DIR_PANGENOME="$(dirname $PATH_FASTA)"
