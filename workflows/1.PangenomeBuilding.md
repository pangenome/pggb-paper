...
sed '1d' $PATH_PANGENOMES_TSV | sort -k 4n | grep athaliana82  | grep chr5 | while read f p s n k G POA O REF; do
  seq 1 1 | while read c; do
    echo $f $p $s $n $k $G $POA $O $REF $c;
...
ls $DIR_BASE/vcfs/tomato/*vcf.gz | while read VCF; do
  NAME=$(basename $VCF .vcf.gz)
  echo $NAME

  # Fix header and apply PanSN
  zcat $VCF | sed '/^##contig=<ID=\([1-9]\|1[0-2]\)>$/d' | sed -e '/^[0-9]\b/ s/^/SL5#1#ch0/' -e '/^[1-9][0-9]/ s/^/SL5#1#ch/' | sed -e 's/\(##contig=<ID=SL5#1#\)\([0-9]\)\(,.*\)/\1ch0\2\3/' -e 's/\(##contig=<ID=SL5#1#\)\([1-9][0-9]\)\(,.*\)/\1ch\2\3/' | bgzip -@ 48 -l 9 > $NAME.pansn.vcf.gz
  tabix $NAME.pansn.vcf.gz
done
