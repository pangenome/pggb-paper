...
$ODGI paths -i $DIR_GRAPHS/$f.og -L | grep GCA_028009825.1 | cut -f 1 > $DIR_GRAPHS/$f.ref-contigs.txt
...
for f in athaliana82.p95.s5000.n82.k47.G700-900-1100.Pasm5.O0001_1 tomato23.p95.s5000.n23.k17.G700-900-1100.Pasm5.O0001_1; do
  sbatch -c 48 -p workers --job-name $(echo $f | cut -f 1 -d '.')-non-ref-ranges --wrap "hostname; cd /scratch; $ODGI paths -i $DIR_GRAPHS/$f.og --non-reference-ranges $DIR_GRAPHS/$f.ref-contigs.txt -t 48 | bedtools sort > $f.non-reference-ranges.bed; bgzip -@ 48 -l 9 $f.non-reference-ranges.bed; mv $f.non-reference-ranges.bed.gz $DIR_GRAPHS"
done
