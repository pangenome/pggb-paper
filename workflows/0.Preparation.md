...
ls *fasta.gz | while read f; do NAME=$(basename $f .fasta.gz | cut -f 1 -d '.'); echo $NAME; zcat $f | sed "s/^>/>$NAME#1#chr/g" | bgzip -@ 48 -l 9 > $NAME.fa.gz; samtools faidx $NAME.fa.gz; done
rm *fasta.gz
