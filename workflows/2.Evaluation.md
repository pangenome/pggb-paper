...
sed '1d' $PATH_PANGENOMES_TSV | sort -k 4n | while read f p s n k REF; do
  seq 1 1 | while read c; do
    ...
    echo sbatch -c $THREADS -p workers -J $(basename "$f" .fa.gz) --wrap "hostname; cd /scratch; /lizardfs/guarracino/pggb-paper/scripts/gfa2evaluation.parallel.sh $PATH_GFA $REF $DIR_EVALUATIONS/$out $THREADS;"
  done
done
...
```shell
cd $DIR_EVALUATIONS
(echo "CHROM\tPOS\tAC\tAF\tLV\tSAMPLE\tCHROMOSOME"; ls athaliana82.chr*_p95.s5000.n82.k47_1/vcfeval/haplo.waved/*/fp.vcf.gz | while read VCF; do
...
(echo "CHROM\tPOS\tSAMPLE\tCHROMOSOME"; ls athaliana82.chr*_p95.s5000.n82.k47_1/vcfeval/haplo.waved/*/fn.vcf.gz | while read VCF; do
  CHR=$(echo $VCF | cut -f 2 -d '.' | cut -f 1 -d '_')
  SAMPLE=$(echo $VCF | rev | cut -f 2 -d '/' | rev)
