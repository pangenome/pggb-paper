...
grep 'athaliana\|tomato\|soy' nucmer-based_evaluation.waved.mean.tsv | grep p95.s10000.n37.k23.G700-900-1100.Pasm5.O0001 | grep soy | awk '
    BEGIN { FS=OFS="\t" }
    {
        # Sum columns
...
ls | grep evaluation -v | cut -f 1 -d '.' | sort | uniq | while read PANGENOME; do
...
```shell
ls $DIR_GRAPHS/*/*gfa | while read GRAPH; do
  DIR_PARENT="$(dirname $GRAPH)"
  NAME=$(echo $GRAPH | rev | cut -f 2 -d '/' | rev)
  PREFIX=$DIR_PARENT/$NAME
...
for SPECIES in athaliana82 tomato23 soy37; do
  ...
  sed '1d' $PATH_PANGENOMES_TSV | sort -k 4n | grep $SPECIES | while read f p s n k G POA O REF; do
    ...
    GRAPH=$PREFIX.gfa
    $PANACUS histgrowth $GRAPH -c bp -q 0,1,0.5,0.1 --groupby-sample -t 48 > $PREFIX.histgrowth.tsv; $PANACUS_VISUALIZE --estimate_growth_params $PREFIX.histgrowth.tsv -s 15 10 > $PREFIX.histgrowth.pdf
  done
done
```
